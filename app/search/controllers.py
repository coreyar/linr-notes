...
from app.__init__ import mbz_q
from spotify_logic import mbz_search_dict
...
def perform_query():
    form_sptfy = SpotifyForm(request.form, prefix="form_sptfy")
    if form_sptfy.validate_on_submit():
        print form_sptfy.data['spotify_uri']
        return redirect('/')
    return render_template('search/search_page.html', form_sptfy=form_sptfy)
